...
def pipeline_dhondt_predicho(
    ...
    """
    print("  [Predicho] Generando predicciones por municipio...")
    df_pred = pipeline_prediccion(modelos, df_2023)

    print("  [Predicho] Agregando a nivel provincial...")
    df_prov = votos_predichos_por_provincia(df_pred)

    print("  [Predicho] Aplicando Ley D'Hondt...")
    ...
    print(f"  [Predicho] Total escaños repartidos: {total}")
    return escanos
...
def pipeline_dhondt_real(
    ...
    """
    print("  [Real] Calculando votos reales por slot...")
    df_prov = votos_reales_por_provincia(df_2023_completo)

    print("  [Real] Aplicando Ley D'Hondt...")
    dict_escanos = escanos_por_provincia(ruta_json_poblacion)
    escanos = dhondt_todas_provincias(df_prov, dict_escanos)

    total = sum(escanos.values())
    print(f"  [Real] Total escaños repartidos: {total}")
    return escanos
...
def pipeline_comparacion_dhondt(
    ...
    escanos_pred = pipeline_dhondt_predicho(modelos, df_2023_completo, ruta_json_poblacion)

    print("\nPaso 2 — Escaños reales (benchmark slots):")
    escanos_real = pipeline_dhondt_real(df_2023_completo, ruta_json_poblacion)
